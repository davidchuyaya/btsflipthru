...
export default function CreateCollectionClient({
    ...
    serverCollectionTypes: Selectable<CollectionTypes>[];
    ...
    useEffect(() => {
        const fetchCollection = async () => {
            ...
            const { collections: fetchedCollection, photocards } = result.data;
            setPhotocardLocked(
                photocards.map((p) => {
    ...
    async function onCreateCollectionType(name: string, index: number) {
        const id = await addCollectionTypeToDB({ name });
        if (id.error) {
            setError(id.error);
            return;
        }
    ...
    async function onCreateCardType(name: string, index: number) {
        const id = await addCardTypeToDB({ name });
        if (id.error) {
            setError(id.error);
            return;
        }
        // Update the form field with the newly created card type
        const newCardType: Selectable<CardTypes> = { name, id: id.data! };
